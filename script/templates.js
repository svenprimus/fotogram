function getGalleryPicture(index) {
    return /*html*/ `
        <button class="picture-button" tabindex="0" 
            ...
            <img class="picture" src=${pathgToGallery + pictureGallery[index].file} 
                alt="${pictureGallery[index].alt}" 
            />
        </button>
        `;
}
...
function getModalPictureContent(index) {
    return /*html*/ `
        <img class="dialog-picture" src=${pathgToGallery + pictureGallery[index].file} alt="${pictureGallery[index].alt}"/>
    `;
}
...
function getModalNavigationContent(index) {
    return /*html*/ `
        ...
        <p aria-label="current image index">${index + 1} / ${pictureGallery.length}</p>
